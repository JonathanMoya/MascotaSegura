...
function enviarCorreo() {
    ...
    if (contador === 0) {
        var jsonSend = {nombre: nombre.val(), departamento: departamento.val(), email: email.val(), telefono: telefono.val(), mensaje: mensaje.val()};
        socket.emit('correoEnviar', jsonSend)
    }
}
socket.on('correoRespuesta', function (response) {
    var nombre = $('#nombre');
    var departamento = $('#departamento');
    var email = $('#email');
    var telefono = $('#telefono');
